...
# 日本語フォント設定
plt.rcParams['font.family'] = ['Noto Sans CJK JP', 'SimHei', 'DejaVu Sans']
...
def main():
    ...
    colors = ['green' if i > 0 and monthly['Growth'].iloc[i] > 0 else 'steelblue' 
              for i in range(len(monthly))]
    ax2.bar(monthly['Month'], monthly['Amount']/1e6, color=colors)
    ax2.set_xlabel('月')
    ...
    with col_a:
        ...
        ax3.hist(rfm['Recency'], bins=15, color='purple', edgecolor='white')
    ...
    with col_b:
        ...
        ax4.hist(rfm['Frequency'], bins=15, color='green', edgecolor='white')
    ...
    with col_c:
        ...
        ax5.hist(rfm['Monetary']/1e6, bins=15, color='blue', edgecolor='white')
    ...
    with col_d:
        ...
        ax6.pie(seg_counts.values, labels=seg_counts.index, autopct='%1.1f%%')
        ax6.set_title('セグメント構成比')
        plt.tight_layout()
        st.pyplot(fig6)
        plt.close()
    ...
    with col_e:
        ...
        ax7.barh(seg_counts.index, seg_counts.values, color='teal')
        ax7.set_xlabel('顧客数')
        ax7.set_title('セグメント別顧客数')
        for i, v in enumerate(seg_counts.values):
            ax7.text(v + 0.5, i, str(v), va='center')
        plt.tight_layout()
        st.pyplot(fig7)
        plt.close()
    ...
    ax8.barh(city_sales.index[::-1], city_sales.values[::-1]/1e6, color='navy')
    ax8.set_xlabel('売上 (Million)')
    ax8.set_title('売上 Top 10 都市')
    plt.tight_layout()
    st.pyplot(fig8)
    plt.close()
    ...
    with col_f:
        ...
        ax9.pie(cat_sales.values, labels=cat_sales.index, autopct='%1.1f%%')
        ax9.set_title('カテゴリ別売上')
        plt.tight_layout()
        st.pyplot(fig9)
        plt.close()
    ...
    with col_g:
        ...
        ax10.barh(prod_sales.index[::-1], prod_sales.values[::-1]/1e6, color='purple')
        ax10.set_xlabel('売上 (Million)')
        ax10.set_title('売上 Top 10 商品')
        plt.tight_layout()
        st.pyplot(fig10)
        plt.close()
    ...
    ax11.plot(cust_val['CustPct'], cust_val['CumPct'], 'b-', linewidth=2)
    ax11.axhline(80, color='red', linestyle='--', label='80%ライン')
    ax11.axvline(20, color='green', linestyle='--', label='20%ライン')
    ax11.fill_between(cust_val['CustPct'], cust_val['CumPct'], alpha=0.2)
    ax11.set_xlabel('顧客累積割合 (%)')
    ax11.set_ylabel('売上累積割合 (%)')
    ax11.legend()
    ...
    # 発見
    st.header("主要発見")
    st.write(f"1. 上位20%顧客が売上の{top20_share:.1f}%を占める")
    st.write(f"2. At Risk顧客: {seg_counts.get('At Risk', 0)}人")
    st.write(f"3. Champions顧客: {seg_counts.get('Champions', 0)}人")
    ...
    st.header("推奨事項")
    st.markdown("""
    1. **リテンション施策:** At Riskセグメント向け特別オファー
    2. **ロイヤリティプログラム:** Champions向けVIP特典
    3. **成長戦略:** Potentialセグメントへのアップセル
    """)
    
    st.markdown("---")
    st.write("RFM顧客分析ダッシュボード - データサイエンス課題 2025")
